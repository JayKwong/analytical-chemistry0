    ...
    while 1:
        try:
            ...
            try:
                ...
                print(f"\t\n极差\tR为\t{jicha}")
def sjdd_yiyuanruosuan():
    """用来分析一元弱酸/碱,大多数时候二元也可以"""
    try:
        c = float(input("请输入酸/碱的浓度或logc\n科学计数法用'xe-n'表示\n\t"))
        if c < 0 :
            c = 1*10**(c)
...
def suanjiandidingfenxi():
    ...
    if yixiang == "1":
        ...
        huanchongrongye()
def geshihuashuru(data):
    """用来格式化数据"""
    data1 = data.split(",")
    lastdata = [float(shuju) for shuju in data1 ]
    content = json.dumps(lastdata)
    path = Path("实验数据1.txt")
...
def shujikeshihua():
    ...
    while active:
        try:
            data = input("请输入要绘制图形的数据\n\t")
            content1 =  geshihuashuru(data)
            fig,ax = plt.subplots()
            shujuyou = 1
            while shujuyou:
                datas = input("是否还有需要对照的数据,有的话请直接输入\n\t")
                if datas:
                    datas = geshihuashuru(datas)
                    ax.plot(datas,linewidth = 5)
                yixaing = input("没有数据了的话请输入'no'\n\t")
                if yixaing == "no" or yixaing == "NO":
                    shujuyou = 0
            ax.plot(content1,linewidth = 5)
            ax.set_xlabel("data_number")
            ax.set_ylabel("data_daxiao")
            ax.set_title("data_name")
            plt.show()
        except ValueError:
            print("输不来数字？")
        yixiang = input("想退出请输入“退出”")
        if yixiang == "退出":
            active = 0   
